            ...
            if left_word and right_word and left_word['status'] == 'active' and right_word['status'] == 'active':
                if left_id == right_id:
                    ...
                    if game_state['active_pairs'] > 1:
                        ...
                        game_state['message'] = f"Все верно! Поздравляем! Ошибок: {game_state['errors']}"
                else:
...
def memo_game(request):
    ...
    if request.method == 'POST':
        ...
        if action == 'reset':
            pairs_to_use = terms_work.select_terms_for_game(selected_category_info)
            request.session['memo_game_state'] = initialize_memo_game_state(pairs_to_use, selected_category_info['id'])
            game_state = request.session['memo_game_state']
